...
class PlantSession:
    ...
    def snapshot(self) -> dict:
        ...
        return {
            ...
            "commissioning": {
                "meter_type": _safe(lambda: inverter.meter_type) if inverter else None,
                "ct_direction_em115_reversed": _safe(lambda: inverter.enable_reversed_115_meter) if inverter else None,
                "ct_direction_em418_reversed": _safe(lambda: inverter.enable_reversed_418_meter) if inverter else None,
                "battery_type": _safe(lambda: inverter.battery_type) if inverter else None,
                "battery_capacity_ah": _safe(lambda: inverter.battery_capacity_ah) if inverter else None,
                "pv_startup_voltage": _safe(lambda: inverter.v_pv_start) if inverter else None,
                "grid_export_limit_w": _safe(lambda: inverter.grid_port_max_power_output) if inverter else None,
                "grid_import_limit_a": _safe(lambda: inverter.grid_import_limit) if inverter else None,
                "grid_import_limit_enabled": _safe(lambda: inverter.grid_import_limit_enabled) if inverter else None,
                "pv_input_mode": _safe(lambda: inverter.mppt_operating_mode) if inverter else None,
                "force_off_grid": _safe(lambda: inverter.force_off_grid) if inverter else None,
            },
            "telemetry": {
